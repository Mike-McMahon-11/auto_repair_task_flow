...
def _weekday_business_days_left(today: date) -> int:
    # M–F remaining until next month (excludes today)
...
def _incoming_filter_for_user(q, user: User):
    my_groups = []
    if user.role == 'estimator': my_groups.append('estimators')
    if user.role == 'parts': my_groups.append('parts')
    if user.role == 'csr': my_groups.append('csr')
    if user.role == 'tech': my_groups.append('tech_pool')
    # porter has no special group, but everyone sees tech_pool:
    if 'tech_pool' not in my_groups:
        my_groups.append('tech_pool')
    return q.filter(or_(Task.assigned_to == user.username, Task.assigned_group.in_(my_groups)))

def _outgoing_filter_for_user(q, user: User):
    return q.filter(
        Task.submitted_by_user_id == user.id,
        or_(Task.assigned_to != user.username, Task.assigned_to.is_(None))
    )

def _self_filter_for_user(q, user: User):
    return q.filter(Task.submitted_by_user_id == user.id, Task.assigned_to == user.username)
...
def create_user():
    if request.method == 'POST':
        ...
        if not all([username,password,shop_id]):
...
def index():
    ...
    shop = get_active_shop(current_user)
    # Auto-archive previous month summary when a new month has started
    _ensure_prev_month_snapshot(shop, date.today())

    tab = request.args.get('tab', 'all')  # all | incoming | outgoing | self

    # EXCLUDE completed tasks from Requests views
    base = Task.query.filter_by(shop_id=shop.id, kind='tech').filter(Task.status != 'done').filter(Task.deleted_at.is_(None))

    if tab == 'incoming':
        rows_q = _incoming_filter_for_user(base, current_user)
    elif tab == 'outgoing':
        rows_q = _outgoing_filter_for_user(base, current_user)
    elif tab == 'self':
        rows_q = _self_filter_for_user(base, current_user)
    else:
        rows_q = base

    rows = rows_q.order_by(Task.updated_at.desc()).limit(200).all()

    # KPIs
    total_tech = base.count()
    vendor_approvals = Task.query.filter_by(shop_id=shop.id, kind='vendor', status='pending').count()
    start, end = _month_range(utcnow())
    done_this_month = Task.query.filter_by(shop_id=shop.id, kind='tech').filter(
        Task.status == 'done', Task.updated_at >= start, Task.updated_at < end
    ).count()
    remaining_to_goal = max((shop.monthly_goal or 0) - done_this_month, 0)
    workdays_left = _weekday_business_days_left(date.today())

    # Read-state for current user
    read_rows = TaskRead.query.filter(
        TaskRead.user_id == current_user.id,
        TaskRead.task_id.in_([t.id for t in rows])
    ).all()
    read_ids = {r.task_id for r in read_rows}
    ...
    return render_template(
        ...
        kpi=dict(
            ...
            incoming=_incoming_filter_for_user(base, current_user).count(),
            outgoing=_outgoing_filter_for_user(base, current_user).count(),
        ),
        read_ids=read_ids,
        roles=ROLES,
...
def change_status(task_id: int):
    ...
    db.session.commit()
    return redirect(url_for('index', tab=request.args.get('tab','all')))
...
def tech_submit_request():
    ...
    if is_multipart:
        # Fields posted as regular form inputs
        req_type = (request.form.get('request_type') or '').strip()
        comments = (request.form.get('comments') or '').strip()
        urgent = (request.form.get('urgent') in ('1','true','True','on'))
        photos = request.files.getlist('photos')  # multiple inputs can share name 'photos'
    else:
        data = request.get_json(silent=True) or {}
        req_type = (data.get('request_type') or '').strip()
        comments = (data.get('comments') or '').strip()
        urgent = bool(data.get('urgent'))
        photos = []  # none in JSON mode

    # Resolve shop
    shop_id = (get_active_shop_id(current_user) or getattr(current_user, 'shop_id', None))
    if not shop_id:
        return jsonify({'ok': False, 'error': 'NoShop'}), 400
    if not req_type:
        return jsonify({'ok': False, 'error': 'Missing request_type'}), 400

    # Map request type -> group
    req = req_type.lower()
    if req in ('blueprint', 'pull parts', 'return parts'):
        assigned_group = 'parts'
    else:
        assigned_group = 'estimators'

    # Optional vendor bits (future use)
    vendor_name = ((request.form.get('vendor_name') if is_multipart else (data.get('vendor_name') if not is_multipart else None)) or '').strip() or None
    sublet_assigned = (request.form.get('sublet_assigned') in ('1','true','True','on')) if is_multipart else bool((data or {}).get('sublet_assigned'))
    ...
    try:
        t = Task(
            ...
            sublet_assigned=sublet_assigned,
        )
        db.session.add(t)
        db.session.commit()
        ...
        app.logger.info("tech_submit_request OK: id=%s shop=%s type=%s", t.id, t.shop_id, t.title)
        return jsonify({'ok': True, 'id': t.id})
    except Exception as e:
        app.logger.exception("tech_submit_request failed")
